...
当前 **186/186** 单元测试全部通过；UI smoke / 截图测试在 `EmptyUITests` 中单独运行。
...
- [x] **ReaderMemory Phase 1/2**（见 [docs/READER-MEMORY-PLAN.md](docs/READER-MEMORY-PLAN.md)）：跨书记忆 ingest/recall、伴读 `recall_reader_memory`、`propose_memory` 确认写入、思维链接走记忆召回路
- [x] **活思维链接基础升级**（见 [docs/LIBER-PORT-PLAN.md](docs/LIBER-PORT-PLAN.md) Wave 1）：链接卡 / 主题记忆可参与 `ThoughtLinkFinder`，AI theme/why 仍按需生成
- [ ] MemoryEmbedding 后台索引、旧问答压缩为 `theme`、伴读镜片（今译 / 辩难 / 文献）、Passkey + CloudKit 记忆同步
