...
| P3-5 | 更简单的前台自动同步壳层 | 已实现：本地 `SyncMutationJournal`、前台自动 pull + 增量 / tombstone push、自动同步开关 / 间隔、待同步变化提示、简化设置引导 |
