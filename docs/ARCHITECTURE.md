...
- [SYNC-BACKUP-DESIGN.md](./SYNC-BACKUP-DESIGN.md) — 本机 reader data 与 `.empty-notes` 读者笔记包边界
...
2. **备份读者笔记，不备份书籍正文** — 高亮、进度、词汇、卡片、ReaderMemory 可导出为 `.empty-notes`；章节文本与 embedding 仅存本机。
...
这个 store 仍沿用历史配置名 `Synced`，只为打开既有本机 SQLite 文件；运行时已经固定 `cloudKitDatabase: .none`。它是 `.empty-notes` 读者笔记包的唯一备份对象。

`AppSession` 只调用 `AppStores.makeContainer(ephemeral:)` 构造本机容器。没有 `SyncSettings`、iCloud 状态探测、自建 server、文件夹同步、live delta、mutation journal、后台调度、冲突策略或 Passkey 账号壳。
...
跨 store 关联：**仅通过 `Book.id`（UUID）**，不使用 SwiftData 跨 store relationship。这个约束确保 EPUB/PDF 文件、章节正文、译文缓存和 embedding 不会进入备份包。
...
| 本地 unsigned build | 可用 `CODE_SIGNING_ALLOWED=NO` 跑测试和调试；当前无云 entitlement 依赖 |
